...
Abstract:
    ...
    Laptop Monitor: derived from microsoft/Windows-driver-samples video/IndirectDisplay (commit 97429c5). Reduced to a
    single monitor exposing only 1920x1080 @ 60 Hz. Frames are still acquired and discarded as in the sample.
...
// Static info about the single monitor reported to the OS
static const struct IndirectSampleMonitor s_SampleMonitors[] =
{
    // EDID 1.4 "LaptopMon" (vendor LMV): one CTA-861 1920x1080@60 detailed timing (148.5 MHz), sRGB.
    //
    // The physical image size is what decides how big Windows draws everything on this monitor. It is declared as
    // 294x166 mm in the detailed timing and 29x17 cm in the basic parameters: a 13.3" 16:9 panel, which puts
    // 1920x1080 at ~166 DPI. Windows derives its *recommended* per-monitor scaling from exactly that, so a fresh
    // connection comes up around 150% instead of the 100% a 24" (527x296 mm) panel would get, without touching
    // the resolution the stream carries. See host/src/display_scale.cpp for the per-display override.
    {
        {
            0x00,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0x00,0x31,0xB6,0x01,0x00,0x01,0x00,0x00,0x00,0x00,0x2E,0x01,
            0x04,0xA0,0x1D,0x11,0x78,0x06,0xEE,0x91,0xA3,0x54,0x4C,0x99,0x26,0x0F,0x50,0x54,0x00,0x00,0x00,
            0x01,0x01,0x01,0x01,0x01,0x01,0x01,0x01,0x01,0x01,0x01,0x01,0x01,0x01,0x01,0x01,0x02,0x3A,0x80,
            0x18,0x71,0x38,0x2D,0x40,0x58,0x2C,0x45,0x00,0x26,0xA6,0x10,0x00,0x00,0x1E,0x00,0x00,0x00,0xFD,
            0x00,0x3B,0x3D,0x42,0x45,0x0F,0x01,0x0A,0x20,0x20,0x20,0x20,0x20,0x20,0x00,0x00,0x00,0xFC,0x00,
            0x4C,0x61,0x70,0x74,0x6F,0x70,0x4D,0x6F,0x6E,0x0A,0x20,0x20,0x20,0x00,0x00,0x00,0x10,0x00,0x00,
...
void IndirectDeviceContext::InitAdapter()
{
    ...
    AdapterCaps.EndPointDiagnostics.pEndPointFriendlyName = L"Laptop Monitor Virtual Display";
    AdapterCaps.EndPointDiagnostics.pEndPointManufacturerName = L"Laptop Monitor";
    AdapterCaps.EndPointDiagnostics.pEndPointModelName = L"LaptopMonitorIdd";
